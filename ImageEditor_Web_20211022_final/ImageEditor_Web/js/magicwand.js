...
var MagicWand = (function () {
    ...
    function floodFillWithBorders(image, px, py, colorThreshold, mask) {
        ...
        resCtx.fillStyle = "#00FF00"
        var _imgData = resCtx.createImageData(resCanvas.width, resCanvas.height)
        ...
        resCtx.putImageData(_imgData, 0, 0)
    ...
    lib.getBorderIndices = function(mask) {
        var _tmpCtx = mask.context
        var x, y, k, k1, k2,
            w = mask.bounds.maxX - mask.bounds.minX,
            h = mask.bounds.maxY - mask.bounds.minY,
            data = _tmpCtx.getImageData(mask.bounds.minX, mask.bounds.minY, w, h).data
            border = [], // only border points
            x1 = w - 1,
            y1 = h - 1;
        ...
        for (y = 1; y < y1; y++)
            for (x = 1; x < x1; x++) {
                ...
                if (data[4 *k + 3] < threshold) continue; // "white" point isn't the border

                // check if any neighbor with a "white" color
                if (data[4 *(k + 1)+3 ] < threshold || data[4 * (k - 1)+3 ] < threshold ||
                    data[4 * k1+3 ] < threshold || data[4 * (k1 + 1)+3 ] < threshold || data[4 * (k1 - 1)+3 ] < threshold ||
                    data[4 * k2+3 ] < threshold || data[4 * (k2 + 1)+3 ] < threshold || data[4 * (k2 - 1)+3 ] < threshold) {
                    //if (data[k + 1] + data[k - 1] + 
                    //    data[k1] + data[k1 + 1] + data[k1 - 1] +
                    //    data[k2] + data[k2 + 1] + data[k2 - 1] == 8) continue;
                    border.push({x: x, y: y});
        ...
        // if the "black" point is adjacent to the boundary of the image, it is a border point
        for (y = 0; y < h; y++)
            if (data[4 * y * w + 3] > threshold)
                border.push({x: 0, y: y});

        for (x = 0; x < w; x++)
            if (data[4 * x + 3] > threshold)
                border.push({x: x, y: 0});

        k = w - 1;
        for (y = 0; y < h; y++)
            if (data[4 * (y * w + k) + 3] > threshold)
                border.push({x: k, y: y});

        k = (h - 1) * w;
        for (x = 0; x < w; x++)
            if (data[4 * (k + x) + 3] > threshold)
                border.push({x: x, y: k});
    ...
    //         data = _tmpCtx.getImageData(0, 0, w, h).data
    //         border = [], // only border points
    //         x1 = w - 1,
    //         y1 = h - 1;
